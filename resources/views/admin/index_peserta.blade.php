...
<div class="flex flex-col space-y-8 px-4 sm:px-0">
    ...
    <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-6">
        <div>
            <h2 class="text-2xl sm:text-3xl font-bold text-slate-800 tracking-tight">Manajemen Peserta</h2>
            <p class="text-slate-400 text-sm font-medium mt-1">Kelola data dan status masa PKL mahasiswa/siswa</p>
        </div>

        <form action="{{ route('admin.peserta.index') }}" method="GET" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full lg:w-auto">
            ...
            <button type="submit" class="bg-blue-600 text-white px-8 py-4 rounded-[1.5rem] font-bold shadow-lg hover:bg-blue-700 transition uppercase text-[10px] tracking-widest">Cari</button>
        </form>
    </div>
    ...
    <div class="overflow-x-auto">
        <div class="flex space-x-2 p-1 bg-slate-100 w-fit rounded-2xl">
            <a href="{{ route('admin.peserta.index', ['status' => 'aktif']) }}" class="px-8 py-3 rounded-xl text-xs font-bold transition whitespace-nowrap {{ $status == 'aktif' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600' }}">Peserta Aktif</a>
            <a href="{{ route('admin.peserta.index', ['status' => 'selesai']) }}" class="px-8 py-3 rounded-xl text-xs font-bold transition whitespace-nowrap {{ $status == 'selesai' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600' }}">Peserta Selesai</a>
        </div>
    </div>

    {{-- Table Data --}}
    <div class="bg-white rounded-[2rem] sm:rounded-[3rem] border border-slate-50 shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
            <table class="min-w-full text-left">
                ...
                <tbody class="divide-y divide-slate-50 text-sm text-slate-600">
                    ...
                    <tr class="hover:bg-slate-50/50 transition">
                        ...
                        <td class="px-6 sm:px-8 py-5 font-bold text-slate-800">{{ $row->user->name }}</td>
                        <td class="px-6 sm:px-8 py-5 font-medium text-blue-500">{{ $row->user->login_id }}</td>
                    ...
                    @empty
                    <tr><td colspan="6" class="px-8 py-20 text-center text-slate-400 italic">Data tidak ditemukan.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
...
{{-- MODAL EDIT PESERTA --}}
<div id="modalEdit" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[150] hidden flex items-start sm:items-center justify-center overflow-y-auto p-4">
    <div class="bg-white rounded-[2rem] sm:rounded-[2.5rem] w-full max-w-2xl p-6 sm:p-10 shadow-2xl max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-8">
            <h3 class="text-xl sm:text-2xl font-black text-slate-800">Edit Data Peserta</h3>
        ...
        <form id="formEditPeserta" method="POST">
            ...
            <button type="submit" class="w-full mt-8 bg-blue-600 text-white py-4 rounded-2xl font-black uppercase tracking-widest shadow-lg hover:bg-blue-700">Simpan Perubahan</button>
        </form>
    </div>
</div>
